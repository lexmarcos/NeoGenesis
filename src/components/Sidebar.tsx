...
import type { DeviceStatus, Effect, Profile } from "@/lib/types";

const EFFECT_ICONS = Object.fromEntries(EFFECTS.map((effect) => [effect.id, effect.icon])) as Record<Effect, typeof Cable>;
...
export function Sidebar({ device, checking, onRefresh, profiles, activeProfile, activeDirty, onSelectProfile }: SidebarProps) {
  return (
    <aside className="flex min-h-0 flex-col border-r border-white/[0.06] bg-black/30 px-5 py-6 backdrop-blur-xl">
      ...
      <div className="mb-8 flex items-center gap-3 px-1">
        ...
        <div>
          <div className="font-display text-[15px] font-semibold tracking-tight">Neo Genesis</div>
          <div className="text-[11px] text-muted-foreground">Console de iluminação</div>
        </div>
      </div>

      <div className="mb-3 px-1 font-display text-[10px] font-semibold uppercase tracking-[0.28em] text-zinc-600">Dispositivo</div>
      ...
      <div className="mb-3 flex items-center justify-between px-1">
        <span className="font-display text-[10px] font-semibold uppercase tracking-[0.28em] text-zinc-600">Perfis</span>
        <span className="rounded-full border border-white/[0.06] px-2 py-0.5 text-[10px] text-zinc-500">10 locais</span>
      </div>
      <nav className="-mr-2 min-h-0 space-y-1 overflow-y-auto pr-2">
        {profiles.map((profile) => {
          ...
          return (
            ...
            >
              {active && <span className="absolute left-0 top-1/2 h-5 w-0.5 -translate-y-1/2 rounded-full bg-primary shadow-[0_0_8px_hsl(var(--primary))]" />}
              ...
              {active && activeDirty && (
                <span className="h-1.5 w-1.5 shrink-0 rounded-full bg-amber-400 shadow-[0_0_6px_rgba(251,191,36,.8)]" title="Alterações não salvas" />
              )}
              <EffectIcon className={cn("h-3.5 w-3.5 shrink-0", active ? "text-primary" : "text-zinc-600 group-hover:text-zinc-500")} />
              <ChevronRight className={cn("h-3.5 w-3.5 shrink-0 transition-opacity", active ? "text-primary opacity-100" : "text-zinc-600 opacity-0 group-hover:opacity-60")} />
            </button>
          );
        })}
      </nav>
